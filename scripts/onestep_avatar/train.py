...
def _master(bundle: dict, path: Path) -> torch.Tensor:
    ...
    A schema-1 bundle holds per-window slices and no master. Readers do not reconstruct it:
    obsolete bundles are regenerated by ``precompute.py --capture-only`` so the capture pass
    remains the tensor's only producer.
    """
