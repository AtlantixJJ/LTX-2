...
def _read_chunk(path: Path, frames: int, device: torch.device, *, spatial_scale: tuple[int, int]) -> torch.Tensor:
    ...
    raw = vr.get_batch(range(frames)); _, h, w, _ = raw.shape; sh, sw = spatial_scale
    raw = raw[:, :(h // sh) * sh, :(w // sw) * sw, :]
...
def _tools(model: RefinerModel, latent: torch.Tensor) -> VideoLatentTools:
    return VideoLatentTools(VideoLatentPatchifier(1), VideoLatentShape.from_torch_shape(latent.shape), 24.0, scale_factors=model.scale_factors)
    ...
    with torch.no_grad(), gpu_model(ImageConditioner(model.paths.video_vae(), DTYPE, device)._build_encoder()) as encoder:
        for clip in clips:
            for n in refine_task.CHUNK_LATENT_FRAMES:
                frames = model.scale_factors.time * (refine_task.CTX_LATENT_FRAMES + n) + 1
                try: encoded.append((clip, n, encoder.tiled_encode(_read_chunk(Path(clip["source"]), frames, device, spatial_scale=(model.scale_factors.height, model.scale_factors.width)), None).cpu()))
